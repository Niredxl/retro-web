...
function Editor(){
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const romBytes = new Uint8Array(event.target.result);
      
      if (coreRef.current) {
        coreRef.current.loadROM(romBytes);
        setIsPaused(false); // Automatically unpause when a game loads!
  ...
  // WASM Bridge logic
 useEffect(() => {
    // 1. Reset state when switching systems
    setIsPaused(true);
    if (requestRef.current) cancelAnimationFrame(requestRef.current);
    
    const loadWasmCore = async () => {
      // 2. Map the dropdown choice to the correct Emscripten files
      const scriptName = activeSystem === 'CHIP8' ? 'retro_core.js' : 'gb_core.js';
      const globalFuncName = activeSystem === 'CHIP8' ? 'createRetroCore' : 'createGbCore';

      // 3. Dynamically inject the script into the page if it isn't there yet
      ...
      if (!script) {
          ...
          // Wait for the browser to finish downloading it
      ...
      try {
        ...
        console.error(`Failed to initialize ${activeSystem}:`, err);
      }
    };

    loadWasmCore();

    // Cleanup function when unmounting or switching cores
    return () => {
      if (requestRef.current) cancelAnimationFrame(requestRef.current);
    };
  }, [activeSystem]);
  ...
  useEffect(() => {
    // We map keyboard keys to jgilchrist's GbButton enum integers.
    // (Typical Order: Right=0, Left=1, Up=2, Down=3, A=4, B=5, Select=6, Start=7)
    // Note: If the buttons feel scrambled, check src/input.h to see his exact enum order!
    const keyMap = {
        'ArrowRight': 0, 'd': 0,
        'ArrowLeft': 1, 'a': 1,
        'ArrowUp': 2, 'w': 2,
    ...
    const handleKeyDown = (e) => {
        ...
        if (keyMap[e.key] !== undefined && coreRef.current && !isPausedRef.current) {
            coreRef.current.buttonPressed(keyMap[e.key]);
        }
    };

    const handleKeyUp = (e) => {
        if (keyMap[e.key] !== undefined && coreRef.current && !isPausedRef.current) {
            coreRef.current.buttonReleased(keyMap[e.key]);
        }
    };
  ...
  <div className="relative z-10 flex flex-col lg:flex-row flex-1 overflow-hidden border-t-2">
    <section className="w-full h-auto lg:w-1/2 bg-primary/95 p-6 lg:p-10 flex flex-col border-r-2 border-black font-mono">
      ...
      <div className="flex flex-wrap gap-6 mb-6 text-sm md:text-base justify-between border-b-1 ">
          ...
          <button onClick={() => fileInputRef.current.click()} className="opacity-80 hover:underline">Import ROM</button>
    ...
    <section className="w-full lg:w-1/2 bg-white/80 p-6 lg:p-10 flex flex-col items-center overflow-y-auto">  
      ...
      <div className={`w-full max-w-md mb-10 border border-[#333] rounded-[2.5rem] p-4 sm:p-6 shadow-xl flex flex-col items-center font-mono transition-colors duration-500 ${
        ...
        <div className="w-full aspect-[4/3] bg-[#2e3b2c] rounded-3xl mb-6 relative flex items-center justify-center p-2 shadow-inner overflow-hidden">
             <canvas 
                ref={canvasRef} 
                width={activeSystem === 'CHIP8' ? 64 : 160} 
                height={activeSystem === 'CHIP8' ? 32 : 144}
        ...
        <select 
                ...
                className="bg-[#1a1005] border border-[#4D361A] text-[#E1AD66] rounded px-2 py-1 mr-4 mb-4 outline-none font-bold"
            >
